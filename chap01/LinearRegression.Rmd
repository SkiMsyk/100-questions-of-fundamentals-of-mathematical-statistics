...
results %>% 
  ...
  geom_histogram(binwidth = .1, position = "identity") + 
  geom_density(aes(x=h0, y=..density..), fill = 'orange', alpha=0.3, show.legend = FALSE) +
...
y <- rnorm(N)
beta <- lsm(x, y)
str_interp("r2 = $[.3f]{r2}, adjusted_r2 = $[.3f]{adjtd_r2}", calc_r2(x, y, beta)) %>% print
```
```{r}
res <- summary(lm(y~x[,2]))
str_interp("r2 = $[.3f]{r2}, adjusted_r2 = $[.3f]{ar2}", list(r2 = res$r.squared, ar2 = res$adj.r.squared)) %>% print
```
...
であり，$\e_i$の分散$\sigma^2$から

$$
\begin{aligned}
V[\bm{x}_\ast \hat\beta] &= E[({\bm x}_\ast \hat{\bm \beta} - E[{\bm x}_\ast \hat{\bm \beta}])^T({\bm x}_\ast \hat{\bm \beta} - E[{\bm x}_\ast \hat{\bm \beta}])] \\
&= 
\end{aligned}
$$
