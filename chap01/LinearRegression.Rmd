...
\begin{aligned}
RSS &= {\bm \e}^T (I-H) {\bm \e} \\
&= [v_1, \ldots, v_{N-p-1}, v_{N-p}, \ldots, v_N] 
  \begin{bmatrix}
...
df <- map_dfc(.x = as.list(df_values), .f = function(df){dchisq(x,df=df)}) %>% 
  ...
  labs(color ="degree of freedom") -> g
...
単純なモデル$y_i = 1 + x_i + \e$，ただし$x_i \sim N(2,1), \e_i \sim N(0,1)$としたモデルを考える．$n=1,\ldots,100$として，$x_i$を一回サンプリングして固定する．その後
...
## $t$分布
$t$分布について簡単に紹介したのち，$t$分布の形状を確認しよう．

いま，２つの独立な確率変数$U \sim N(0,1)$と$V \sim \chi^2_m$を考える．この時$T := U/\sqrt{V/m}$という確率変数$T$が従う分布が自由度$m$の$t$分布になる．
