...
$$
\begin{aligned}
(\hat{\bm \beta} - {\bm \beta}) ({\bm y} - {\hat {\bm y}})^T &= (X^TX)^{-1} X^T {\bm \e} {\bm \e}^T (I-H)
\end{aligned}
$$
...
$$
\begin{aligned}
E[(\hat{\bm \beta} - {\bm \beta}) ({\bm y} - {\hat {\bm y}})^T] &= \sigma^2 I E[(X^TX)^{-1} X^T(I-H)] \\
&= \sigma^2 I E[(X^TX)^{-1} X^T - (X^TX)^{-1} X^TH] \\
&= 0
\end{aligned}
$$

となる．ここで，$HX=X \Leftrightarrow X^TH = X$を用いた．また，${\bm y} - {\bar {\bm y}} = (I-H){\bm \e}$は正規分布に従うこと，$\hat{\bm \beta}$が正規分布に従うことから$\hat{\bm \beta} - {\bm \beta}$も正規分布に従うので，正規分布の性質から，$(\hat{\bm \beta} - {\bm \beta}) ({\bm y} - {\hat {\bm y}})^T$は互いに独立であることがわかる．

$U$は$\hat{\bm \beta} - {\bm \beta}$の関数, $V$は$RSS$，すなわち$\bm y - \hat{\bm  y}$の関数であるので，上記の理由により$U,V$は互いに独立であることが導かれた．
...
## 実験：$\hat{\bm \beta}$の分布  
上記の要領で$\hat{\bm \beta}$の推定を繰りかえし，$\hat{\beta_1}/SE(\hat \beta_1)$のヒストグラムを描く．
...
$$
\begin{aligned}
RSS &:= \| \hat{\bm y} - \bm y \|^2 = \| (I-H){\bm y} \|^2 \\
ESS &:= \| \hat{\bm y} - \bar{\bm y} \|^2 = \| (H-W){\bm y} \|^2 \\
TSS &:= \| \bm y - \bar{\bm y} \|^2 = \| (I-W) \bm y  \|^2
\end{aligned}
$$
...
$$
\hat p := \frac{(\bm x - \bar x)^T(\bm y - \bar y)}{\sqrt{\sum_{i=1}^{N}(x_i - \bar x)^2 \sum_{i=1}^{N}(y_i - \bar y)}}
$$

$$
\begin{aligned}
\frac{ESS}{TSS} &= \frac{\hat{\beta_1} \| \bm x - \bar x \|^2}{\| \bm y - \bar y \|^2} \\
...
# 信頼区間と予測区間  
ここからは$\hat{\bm \beta}$の分布を利用して信頼区間について考えていく．$t$の従う分布の確率密度関数を$f$とおいた時，
...
$\bm x_\ast \hat{\bm \beta}$を，推定に用いた$\bm x$とは異なる点$\bm x_\ast \in \R^{p+1}$とおく．この時，$\bm x_\ast \hat{\bm \beta}$の平均

$$
E[\bm x_\ast \hat{\bm \beta}] = \bm x_\ast E[\hat{\bm \beta}]
$$
...
$$
\begin{aligned}
V[\bm{x}_\ast \hat{\bm \beta}] &= {\bm x_\ast} V(\hat{\bm \beta}) {\bm x_\ast}^T \\
&= \sigma^2 {\bm x}_\ast (X^TX)^{-1} {\bm x}_\ast^T
\end{aligned}
$$

を得る．ここで，$\hat \sigma$と$SE({\bm x}_\ast \hat{\bm \beta})$をそれぞれ

$$
\begin{aligned}
\hat \sigma &:= \sqrt{RSS/(N-p-1)} \\
SE({\bm x}_\ast \hat{\bm \beta}) &:= \hat \sigma \sqrt{{\bm x}_\ast (X^TX)^{-1} {\bm x}_\ast^T}
\end{aligned}
$$
...
$$
\begin{aligned}
C &:= \frac{{\bm x}_\ast \hat{\bm \beta} - {\bm x}_\ast {\bm \beta}}{SE({\bm x}_\ast \hat{\bm \beta})} \\
&= \frac{{\bm x}_\ast \hat{\bm \beta} - {\bm x}_\ast {\bm \beta}}{\sigma \sqrt{{\bm x}_\ast (X^TX)^{-1} {\bm x}_\ast^T}} \left/ \sqrt{\left. \frac{RSS}{\sigma^2} \right/ (N-p-1) } \right. \\
& \sim t_{N-p-1}
\end{aligned}
$$

が得られる．$\frac{{\bm x}_\ast \hat{\bm \beta} - {\bm x}_\ast {\bm \beta}}{\sigma \sqrt{{\bm x}_\ast (X^TX)^{-1} {\bm x}_\ast^T}}$は${\bm x}_\ast \hat{\bm \beta}$の平均を引いた物をその標準偏差で割っている形になっているので標準正規分布に従う．分母の部分は$RSS/\sigma^2 \sim \chi^2_{N-p-1}$であり，$RSS, \hat{\bm \beta} - {\bm \beta}$が独立であることがわかっているので，$t_{N-p-1}$に従うことがわかる．


また，観測誤差$\bm \e$を考慮した信頼区間を予測区間$prediction interval$と呼ぶことが多い．この場合においても分散が評価できて，その場合は単純に観測誤差$\bm \e$の分散$\sigma^2$を足すだけである．この場合は

$$
\begin{aligned}
P &:= \frac{ {\bm x}_\ast \hat{\bm \beta} - {\bm y}_\ast}{\sigma (1 + \sqrt{{\bm x}_\ast (X^TX)^{-1} {\bm x}_\ast})} \left/
...
$$
\begin{aligned}
{\bm x}_\ast {\bm \beta} = {\bm x}_\ast \hat{\bm \beta}  \pm t_{N-p-1}(\alpha/2) \hat \sigma \sqrt{{\bm x}_\ast (X^TX)^{-1} {\bm x}_\ast^T}
\end{aligned}
$$
となる．予測区間は
...
$$
\begin{aligned}
{\bm y}_\ast = {\bm x}_\ast \hat{\bm \beta}  \pm t_{N-p-1}(\alpha/2) \hat \sigma \sqrt{1 + {\bm x}_\ast (X^TX)^{-1} {\bm x}_\ast^T}
\end{aligned}
$$
